...
class SRData(data.Dataset):
    def __init__(self, args, name='', train=True, benchmark=False):
        ...
        if args.ext.find('img') >= 0 or benchmark:
            ...
            for s in self.scale:
                for i in range(len(self.dir_lr)):
                    os.makedirs(
                        os.path.join(
                            self.dir_lr[i].replace(self.apath, path_bin),
                            'X{}'.format(s)
                        ),
                        exist_ok=True
                    )
...
#             exit(True)
    ...
    def _scan(self):
        ...
        names_hr = sorted(names_hr * len(self.dir_lr))
...
#         print("dir_lr",self.dir_lr)
        print("we train with ", self.args.use_data.split("_"), "datasets")
        assert len(self.dir_lr) > 1, "check your 'use_data'"
